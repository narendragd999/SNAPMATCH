...
def upgrade():
    ...
    if not index_exists('testimonials', 'ix_testimonials_id'):
        op.create_index('ix_testimonials_id', 'testimonials', ['id'], unique=False)

    # Create faqs table
    if not table_exists('faqs'):
        op.create_table(
            'faqs',
            sa.Column('id', sa.Integer(), nullable=False),
    ...
    if not index_exists('faqs', 'ix_faqs_id'):
        op.create_index('ix_faqs_id', 'faqs', ['id'], unique=False)

    # Create newsletter_subscribers table
    if not table_exists('newsletter_subscribers'):
        op.create_table(
    ...
    if not index_exists('newsletter_subscribers', 'ix_newsletter_subscribers_id'):
        op.create_index('ix_newsletter_subscribers_id', 'newsletter_subscribers', ['id'], unique=False)
    if not index_exists('newsletter_subscribers', 'ix_newsletter_subscribers_email'):
        op.create_index('ix_newsletter_subscribers_email', 'newsletter_subscribers', ['email'], unique=True)

    # Seed default testimonials (only if table is empty)
    if table_exists('testimonials') and not table_has_data('testimonials'):
        op.execute("""
    ...
    if table_exists('faqs') and not table_has_data('faqs'):
        op.execute("""
