...
export default function PublicSelfiePage() {
  ...
  // Guest upload always visible — organizer controls via backend permissions
  ...
  const loadNextPage = useCallback(async () => {
    ...
    if (!resultId || t.loading || !t.has_more) return;
    ...
    try {
      ...
      setTabs(prev => ({ ...prev, [activeTab]: { items: [...prev[activeTab].items, ...data.items], page: data.page, total: data.total, has_more: data.has_more, loading: false, error: null } }));
              ...
              {!resultId && !processing && (
                ...
                >
                  ...
                  <motion.div variants={fadeUp} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24, maxWidth: 440, margin: "0 auto 24px" }}>
                    ...
                    <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={e => { if (e.target.files?.[0]) handleUpload(e.target.files[0]); if (fileInputRef.current) fileInputRef.current.value = ""; }} style={{ display: "none" }} />
                  </motion.div>
              ...
              {resultId && !processing && (
                <motion.section id="results-section" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ maxWidth: 1200, margin: "0 auto", padding: "20px 16px 0" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
                      ...
                      {(["you", "friends"] as const).map(t => (
                        <button
                          ...
                          style={{
                            ...
                            border: "1px solid var(--border)", background: "transparent", color: activeTab === t ? undefined : "var(--text-dim)",
                            transition: "all 0.2s",
                          }}
                        >
                          {t === "you" ? "Your Photos" : "Friends Nearby"}
                          {tabs[t].total > 0 && (
                            <span style={{ marginLeft: 7, padding: "2px 7px", borderRadius: 99, background: "rgba(255,255,255,0.08)", fontSize: 11, fontWeight: 700 }}>
                              {tabs[t].total}
                            </span>
                          )}
                        </button>
                    ...
                    <motion.button
                      whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                      onClick={() => { setResultId(null); setTabs({ you: emptyTab(), friends: emptyTab() }); }}
                      className="btn-ghost"
                  ...
                  {currentItems.length > 0 ? (
                    <>
                      ...
                      >
                        {currentItems.map((item, idx) => (
                          <motion.div
                            ...
                            onClick={() => { setPreviewIndex(idx); setPreviewImage(nameOf(item)); }}
                          >
                            <img src={`${API}/public/events/${token}/image/${nameOf(item)}`} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                            <div className="photo-overlay" style={{ position: "absolute", inset: 0, display: "flex", alignItems: "flex-end", justifyContent: "space-between", padding: "10px 10px 10px" }}>
                              <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "32px 0" }}>
                        ...
                        {!tab.loading && !tab.has_more && tab.total > 0 && <p style={{ color: "var(--text-muted)", fontSize: 12 }}>All {tab.total} photos loaded ✓</p>}
                        {tab.error && (
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 20px", gap: 16, textAlign: "center" }}>
                      ...
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => { setResultId(null); setTabs({ you: emptyTab(), friends: emptyTab() }); }} className="btn-gold" style={{ padding: "10px 20px", borderRadius: 10, fontSize: 13, cursor: "pointer" }}>
                        Try Again
                      </motion.button>
                    </motion.div>
                  )}
            ...
            <motion.div initial={{ scale: 0.92, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.92, y: 16 }}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16, borderTop: "1px solid var(--border)", paddingTop: 24 }}>
                {[
                  ...
                  { icon: Download,  n: "03", title: "Browse & download",   desc: "Scroll your matches, download any photo" },
                  ...(guestUpload ? [{ icon: CloudUpload, n: "04", title: "Share your photos", desc: "Contribute shots for organizer review" }] : []),
                ].map(step => {
                  const Icon = step.icon;
                  return (
